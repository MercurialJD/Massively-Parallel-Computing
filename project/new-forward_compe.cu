...
__constant__ __half2 const_device_mask[3200];     // Size based on the cout mask size


__global__ void conv_forward_kernel(__half2* output, const __half2* input,
    ...
    const int h = (blockIdx.y / W_grid) * TILE_WIDTH + threadIdx.y;
    const int w = (blockIdx.y % W_grid) * TILE_WIDTH + threadIdx.x;
    const int b = blockIdx.z;

    #define out_4d(i3, i2, i1, i0) output[(i3) * (Map_out * Height_out * Width_out) + (i2) * (Height_out * Width_out) + (i1) * (Width_out) + (i0)]
    #define in_4d(i3, i2, i1, i0) input[(i3) * (Channel * Height * Width) + (i2) * (Height * Width) + (i1) * (Width) + (i0)]
    #define mask_4d(i3, i2, i1, i0) const_device_mask[(i3) * (Channel * K * K) + (i2) * (K * K) + (i1) * (K) + (i0)]

    #define acc_once(b, c, h, w, p, q)  acc = __hfma2( in_4d((b), (c), ((h) + (p)), ((w) + (q))), mask_4d((m), (c), (p), (q)), acc );
    #define acc_all_w(b, c, h, w, p)    acc_once(b, c, h, w, p, 0) \
                                        acc_once(b, c, h, w, p, 1) \
                                        acc_once(b, c, h, w, p, 2) \
                                        acc_once(b, c, h, w, p, 3) \
                                        acc_once(b, c, h, w, p, 4) \
                                        acc_once(b, c, h, w, p, 5) \
                                        acc_once(b, c, h, w, p, 6)
    #define acc_all_h_w(b, c, h, w)     acc_all_w(b, c, h, w, 0) \
                                        acc_all_w(b, c, h, w, 1) \
                                        acc_all_w(b, c, h, w, 2) \
                                        acc_all_w(b, c, h, w, 3) \
                                        acc_all_w(b, c, h, w, 4) \
...
__host__ void GPUInterface::conv_forward_gpu_prolog(const float *host_output, const float *host_input, const float *host_mask, float **device_output_ptr, float **device_input_ptr, float **device_mask_ptr, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    cudaMemcpyToSymbol(const_device_mask, half_host_mask, mask_size);
...
__host__ void GPUInterface::conv_forward_gpu(float *device_output, const float *device_input, const float *device_mask, const int Batch, const int Map_out, const int Channel, const int Height, const int Width, const int K)
{
    ...
    conv_forward_kernel<<<dimGrid, dimBlock>>>((__half2 *) device_output, (__half2 *) device_input, \
